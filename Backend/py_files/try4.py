...
with open('dataset/keywords/english_keywords.json') as json_file:
    mappings = json.load(json_file)
stopwords=set(stopwords)-set(['between','more','how','many','number','of'])
def read_data_english(folderPath,exercise=0):
    ...
    if(exercise==1):
        ...
        for item in path.rglob('*.pdf'):
            ...
            for count, img in enumerate(doc):
                ...
                extracted_text = pytesseract.image_to_string(Image.open(os.path.join(temp_path, img_name + ".png")),lang="eng",config='--psm 6')
        ...
        df = df.transpose()
        print(df)
        df.to_csv("Backend/py_files/english_exercise.csv")
        with open("Backend/py_files/english_exercises_dataset.json", "w") as outfile:
        ...
        df = df.transpose()
        print(df)
        df.to_csv("Backend/py_files/english_lecture_csv.csv")
        with open("Backend/py_files/english_lecture_dataset.json", "w") as outfile:
...
class RecommendationDetails:
    ...
    #dataFolderPath = 'dataset/Test/english/lecture/'
 #   exerciseFolderPath = 'dataset/Test/exercise/'
   # read_data_english(dataFolderPath)
